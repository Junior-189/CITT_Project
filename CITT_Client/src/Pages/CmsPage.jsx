...
const CmsPage = () => {
  ...
  const fetchPage = async () => {
    ...
    setError("");
    try {
      const res = await api.get(`/api/cms/public/pages/${slug}`);
      setPage(res.data);
    } catch (err) {
        ...
        <div className="max-w-3xl mx-auto">
          <div className="flex items-center justify-center min-h-[400px]">
            <div className="text-center">
              ...
              <p className="text-slate-600 dark:text-slate-400 text-lg">Loading page...</p>
            </div>
          </div>
        </div>
      </main>
        ...
        <div className="max-w-3xl mx-auto">
          <div className="bg-white dark:bg-slate-800 rounded-xl p-12 shadow-md text-center">
            ...
            <p className="text-slate-500 dark:text-slate-400">
              The page you are looking for does not exist or is not yet published.
            </p>
          </div>
        </div>
      </main>
  ...
  return (
    <main className="flex-1 px-4 md:px-16 py-10 overflow-auto bg-gray-50 dark:bg-slate-900 text-slate-800 dark:text-slate-100">
      <article className="max-w-3xl mx-auto">
        ...
        <p className="text-sm text-slate-400 dark:text-slate-500 mb-8 pb-8 border-b border-slate-200 dark:border-slate-700">
