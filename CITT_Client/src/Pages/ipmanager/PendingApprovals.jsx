      ...
      <div className="max-w-7xl mx-auto">
        ...
        {pendingIPs.length === 0 ? (
          <div className="bg-white dark:bg-slate-800 rounded-lg shadow-md p-12 text-center">
            ...
            <h3 className="mt-4 text-lg font-medium text-gray-900 dark:text-slate-100">No Pending Applications</h3>
            <p className="mt-2 text-sm text-gray-600 dark:text-slate-400">All IP applications have been reviewed. New applications will appear here.</p>
          </div>
        ) : (
          <div className="space-y-4">
            {pendingIPs.map((ip, index) => (
              <div key={ip.id} className="bg-white dark:bg-slate-800 rounded-lg shadow-md p-6 border-l-4 border-yellow-500">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500 dark:text-slate-400">Application ID: {ip.id}</p>
                    {ip.ip_type && (
                      <div className="mt-2">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-sm font-medium text-gray-900 dark:text-slate-100">{formatDate(ip.created_at)}</p>
                  </div>
                </div>

                <div className="grid md:grid-cols-3 gap-4 mb-4">
                  <div>
                    <p className="text-sm font-semibold text-gray-700 dark:text-slate-300">Applicant</p>
                  ...
                  <div>
                    <p className="text-sm font-semibold text-gray-700 dark:text-slate-300">Field</p>
                    <p className="text-sm text-gray-600 dark:text-slate-400 mt-1">{ip.field || 'N/A'}</p>
                  </div>
                </div>

                {ip.trl && (
                  <div className="mb-4">
                    <p className="text-sm font-semibold text-gray-700 dark:text-slate-300">TRL Level</p>
                    <p className="text-sm text-gray-600 dark:text-slate-400 mt-1">{ip.trl}</p>
                  </div>
                )}
                ...
                {(ip.abstract || ip.description) && (
                  <div className="mb-4">
                    ...
                    <p className="text-sm text-gray-600 dark:text-slate-400 mt-1">{ip.abstract || ip.description}</p>
                  </div>
                )}

                {ip.prior_art && (
                  <div className="mb-4">
                    <p className="text-sm font-semibold text-gray-700 dark:text-slate-300">Prior Art</p>
                    <p className="text-sm text-gray-600 dark:text-slate-400 mt-1">{ip.prior_art}</p>
                  </div>
                )}
            ...
            <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
              ...
              <div className="inline-block align-bottom bg-white dark:bg-slate-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <div className="bg-white dark:bg-slate-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                  <h3 className="text-lg leading-6 font-medium text-gray-900 dark:text-slate-100 mb-4">Approve IP Application</h3>
        ...
        {showRejectModal && selectedIP && (
          <div className="fixed z-50 inset-0 overflow-y-auto">
            <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
              ...
              <div className="inline-block align-bottom bg-white dark:bg-slate-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <div className="bg-white dark:bg-slate-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                  <h3 className="text-lg leading-6 font-medium text-gray-900 dark:text-slate-100 mb-4">Reject IP Application</h3>
                  <p className="text-sm text-gray-500 dark:text-slate-400 mb-4">
                    Application: <strong>{selectedIP.title || selectedIP.ip_title || 'Untitled'}</strong>
                  </p>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">Rejection Reason *</label>
                    <textarea
