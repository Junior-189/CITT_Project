              ...
              <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                <thead className="bg-gray-50 dark:bg-slate-900">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">Applicant</th>
                ...
                <tbody className="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700">
                  {filteredIps.map((ip) => (
                    <tr key={ip.id} className="hover:bg-gray-50 dark:hover:bg-slate-700">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900 dark:text-slate-100">{ip.user_name || 'Unknown'}</div>
                        <div className="text-xs text-gray-500 dark:text-slate-400">{ip.user_email}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm text-gray-900 dark:text-slate-100">{ip.ip_type || 'N/A'}</span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
              ...
              <div className="inline-block align-bottom bg-white dark:bg-slate-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
                <div className="bg-white dark:bg-slate-800 px-4 pt-5 pb-4 sm:p-6">
                  ...
                  <div className="space-y-3 mb-6">
                    <div><p className="font-semibold text-gray-700 dark:text-slate-300 text-sm">Title</p><p className="text-gray-900 dark:text-slate-100">{selectedIP.title || selectedIP.ip_title || 'Untitled'}</p></div>
                    <div className="grid grid-cols-2 gap-4">
                      <div><p className="font-semibold text-gray-700 dark:text-slate-300 text-sm">Applicant</p><p className="text-gray-900 dark:text-slate-100">{selectedIP.user_name || 'N/A'}</p></div>
                      <div><p className="font-semibold text-gray-700 dark:text-slate-300 text-sm">IP Type</p><p className="text-gray-900 dark:text-slate-100">{selectedIP.ip_type || 'N/A'}</p></div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div><p className="font-semibold text-gray-700 dark:text-slate-300 text-sm">Inventors</p><p className="text-gray-900 dark:text-slate-100">{selectedIP.inventors || 'N/A'}</p></div>
                      <div><p className="font-semibold text-gray-700 dark:text-slate-300 text-sm">Field</p><p className="text-gray-900 dark:text-slate-100">{selectedIP.field || 'N/A'}</p></div>
                    </div>
                    {(selectedIP.abstract || selectedIP.description) && (
                      <div><p className="font-semibold text-gray-700 dark:text-slate-300 text-sm">Description</p><p className="text-gray-600 dark:text-slate-400 text-sm">{selectedIP.abstract || selectedIP.description}</p></div>
                    )}
