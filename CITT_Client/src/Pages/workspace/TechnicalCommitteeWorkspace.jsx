...
import { CheckCircle, XCircle, MessageSquare, Bell, LayoutDashboard, FolderKanban, ChevronDown, ChevronUp, Download } from 'lucide-react';
...
const ProjectDetailModal = ({ projectId, onClose, theme }) => {
  ...
  const handleComment = async (stageNum) => {
    ...
    try {
      ...
      await api.post(`/api/workspace/project/${projectId}/comment`, {
        stage_number: stageNum, comment, comment_type: 'feedback',
      });
                    ...
                    <div className="px-5 pb-5 border-t border-gray-100 pt-4 space-y-4">
                      ...
                      {isSubmitted && (
                        <div className="space-y-3 p-4 bg-gray-50 dark:bg-slate-900 rounded-xl">
                          <p className="text-sm font-bold text-slate-700 dark:text-slate-200 dark:text-slate-200">Technical Evaluation:</p>
                          <div>
                            <label className="text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1 block">Approval / Evaluation Notes *</label>
                            <textarea
                              rows={2}
                          ...
                          <div className="border-t border-gray-200 dark:border-slate-700 pt-3">
                            <label className="text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1 block">Request Revision Reason *</label>
                            <textarea
                              rows={2}
                              value={rejectNotes[stageNum] || ''}
                              onChange={e => setRejectNotes(n => ({ ...n, [stageNum]: e.target.value }))}
                              placeholder="Explain the technical issues..."
                      ...
                      {stageComments.length > 0 && (
                        <div className="space-y-2">
                          <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">Evaluation Notes</p>
                          {stageComments.map(c => (
                      ...
                      <div className="border-t border-gray-100 pt-3">
                        <p className="text-xs font-bold text-slate-500 mb-2">Technical Evaluation Notes</p>
                        <textarea
                          rows={2}
...
const TechnicalCommitteeWorkspace = () => {
  const { getAuthenticatedAxios } = useAuth();
  ...
  const TABS = [
    ...
    { key: 'evaluations', label: 'Evaluations', icon: FolderKanban },
              ...
              <div className="space-y-4">
                ...
                {projects.filter(p => Number(p.pending_review) > 0).length === 0 ? (
                  ...
                  projects.filter(p => Number(p.pending_review) > 0).map(p => (
                    <ProjectCard key={p.id} project={p} onReview={() => setSelectedProject(p.id)} />
                  ))
                )}
              </div>
            )}
...
export default TechnicalCommitteeWorkspace;
