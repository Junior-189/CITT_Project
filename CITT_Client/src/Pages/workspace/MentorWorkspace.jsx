...
import { CheckCircle, XCircle, Clock, MessageSquare, Bell, LayoutDashboard, FolderKanban, ChevronDown, ChevronUp, Download } from 'lucide-react';
...
const ProjectDetailModal = ({ projectId, onClose, theme, commentLabel, commentType }) => {
  ...
  const getMilestone = (stageNum) => data?.milestones?.find(m => m.stage_number === stageNum);
  const getStageComments = (stageNum) => data?.comments?.filter(c => c.stage_number === stageNum) || [];
  ...
  const handleApprove = async (stageNum) => {
    ...
    try {
      ...
      setMessage(`Stage ${stageNum} approved successfully.`);
      setApproveNotes(n => ({ ...n, [stageNum]: '' }));
      await fetchDetail();
    } catch (e) { setMessage(e.response?.data?.error || 'Approval failed.'); }
  ...
  const handleReject = async (stageNum) => {
    ...
    try {
      ...
      await fetchDetail();
    } catch (e) { setMessage(e.response?.data?.error || 'Rejection failed.'); }
  ...
  const handleComment = async (stageNum) => {
    ...
    if (!comment) return setMessage('Comment cannot be empty.');
    setProcessing(p => ({ ...p, [`comment_${stageNum}`]: true }));
    try {
      const api = getAuthenticatedAxios();
      await api.post(`/api/workspace/project/${projectId}/comment`, {
        stage_number: stageNum, comment, comment_type: commentType || 'feedback',
      });
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-start justify-center p-4 overflow-y-auto">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-4xl my-8">
        ...
        <div className="flex items-start justify-between p-6 border-b border-gray-100">
          <div>
            <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 dark:text-slate-100">{data?.project?.title || 'Loading...'}</h2>
            <p className="text-sm text-gray-400 mt-0.5">
              {data?.project?.innovator_name} · {data?.project?.category}
            </p>
          </div>
          ...
          <div className="p-6 space-y-3">
            {message && (
              <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg text-sm text-blue-700 mb-4">
                {message}
                <button onClick={() => setMessage('')} className="ml-2 text-xs underline">Dismiss</button>
              </div>
            )}

            {/* Team */}
            {data?.team?.length > 0 && (
              <div className="p-4 bg-gray-50 dark:bg-slate-900 rounded-xl mb-4">
                <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-2">Assigned Team</p>
                <div className="flex flex-wrap gap-2">
                  {data.team.map(m => (
                    <span key={m.id} className="text-xs bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-full px-3 py-1 text-slate-600 dark:text-slate-400 dark:text-slate-400">
                      {m.member_name} <span className="text-gray-400">({m.member_role})</span>
                    </span>
                  ))}
                </div>
              </div>
                ...
                <div key={stageNum} className={`border rounded-xl overflow-hidden ${isSubmitted ? 'border-blue-300 shadow-sm' : 'border-gray-200'}`}>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${theme.circle}`}>
                        {stageNum}
                      </span>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {stageComments.length > 0 && (
                        <span className="text-xs text-gray-400">{stageComments.length} comment{stageComments.length > 1 ? 's' : ''}</span>
                      )}
                    ...
                    <div className="px-5 pb-5 border-t border-gray-100 pt-4 space-y-4">
                      ...
                      {milestone?.submission_notes && (
                        <div className="p-4 bg-blue-50 rounded-xl">
                          <p className="text-xs font-bold text-blue-600 uppercase tracking-wide mb-2">Innovator's Submission</p>
                      ...
                      {milestone?.approval_notes && status === 'completed' && (
                        <div className="p-3 bg-green-50 rounded-lg">
                          <p className="text-xs font-bold text-green-600 mb-1">Approval Notes:</p>
                          <p className="text-sm text-green-700">{milestone.approval_notes}</p>
                        </div>
                      )}

                      {/* Approve / Reject for submitted stages */}
                      {isSubmitted && (
                        <div className="space-y-3 p-4 bg-gray-50 dark:bg-slate-900 rounded-xl">
                          <p className="text-sm font-bold text-slate-700 dark:text-slate-200 dark:text-slate-200">Review this submission:</p>
                          <div>
                            <label className="text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1 block">Approval Notes *</label>
                            <textarea
                              rows={2}
                              value={approveNotes[stageNum] || ''}
                              onChange={e => setApproveNotes(n => ({ ...n, [stageNum]: e.target.value }))}
                              placeholder="Write approval notes..."
                          ...
                          <div className="border-t border-gray-200 dark:border-slate-700 pt-3">
                            <label className="text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1 block">Rejection Reason *</label>
                            <textarea
                              rows={2}
                              ...
                              className="w-full border border-gray-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm resize-none focus:ring-2 focus:ring-red-200 focus:outline-none"
                            />
                            <button
                      ...
                      {stageComments.length > 0 && (
                        <div className="space-y-2">
                          ...
                          {stageComments.map(c => (
                            <div key={c.id} className="p-3 bg-white dark:bg-slate-800 border border-gray-100 rounded-lg">
                              <div className="flex items-center gap-2 mb-1">
                                <span className="text-xs font-semibold text-slate-700 dark:text-slate-200 dark:text-slate-200">{c.commenter_name}</span>
                                <span className="text-xs text-gray-400">({c.commenter_role})</span>
                                <span className={`text-xs px-1.5 py-0.5 rounded-full ${STATUS_COLORS[c.comment_type] || 'bg-gray-100 text-gray-600'}`}>
                                  {c.comment_type}
                                </span>
                                <span className="text-xs text-gray-400 ml-auto">{new Date(c.created_at).toLocaleDateString()}</span>
                              </div>
                              <p className="text-sm text-slate-600 dark:text-slate-400 dark:text-slate-400">{c.comment}</p>
                            </div>
                          ))}
                        </div>
                      )}
                      ...
                      <div className="border-t border-gray-100 pt-3">
                        <p className="text-xs font-bold text-slate-500 mb-2">{commentLabel || 'Add Guidance'}</p>
                        <textarea
                          rows={2}
                          value={commentText[stageNum] || ''}
                          onChange={e => setCommentText(t => ({ ...t, [stageNum]: e.target.value }))}
                          placeholder="Write your guidance or feedback..."
                          className="w-full border border-gray-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm resize-none focus:ring-2 focus:ring-teal-300 focus:outline-none"
                        />
                        <button
                          onClick={() => handleComment(stageNum)}
                          disabled={processing[`comment_${stageNum}`]}
                          className={`mt-2 flex items-center gap-2 px-4 py-2 ${theme.btn} text-white rounded-lg text-sm font-semibold disabled:opacity-50`}
                        >
                          <MessageSquare className="w-4 h-4" />
                          {processing[`comment_${stageNum}`] ? 'Posting...' : 'Post Comment'}
                        </button>
                      </div>
                    </div>
                  )}
...
const MentorWorkspace = () => {
  ...
  const theme = {
    ...
    header: 'from-teal-800 to-teal-600',
  };
  ...
  return (
    <main className="flex-1 bg-gray-50 dark:bg-slate-900 overflow-auto">
      {/* Header */}
      <div className={`bg-gradient-to-r ${theme.header} text-white px-6 py-8`}>
        <h1 className="text-2xl font-bold mb-1">Mentor Workspace</h1>
        <p className="text-teal-100 text-sm">Review milestone submissions, provide guidance, and track innovator progress.</p>
      </div>
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-3.5 text-sm font-semibold border-b-2 transition-colors
                ${tab === t.key ? `border-teal-600 ${theme.primary}` : 'border-transparent text-gray-500 hover:text-slate-700'}`}
            >
          ...
          <>
            ...
            {tab === 'overview' && (
              <div className="space-y-4">
                <h2 className="text-lg font-bold text-slate-700 dark:text-slate-200 dark:text-slate-200">Projects Requiring Review</h2>
                {projects.filter(p => Number(p.pending_review) > 0).length === 0 ? (
                  <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 p-10 text-center">
                    ...
                    <p className="text-gray-400">No pending reviews at this time.</p>
                  </div>
                ) : (
                  projects.filter(p => Number(p.pending_review) > 0).map(p => (
                    <ProjectCard key={p.id} project={p} theme={theme} onReview={() => setSelectedProject(p.id)} />
                  ))
                )}
              </div>
            )}
            ...
            {tab === 'projects' && (
              <div className="space-y-4">
                <h2 className="text-lg font-bold text-slate-700 dark:text-slate-200 dark:text-slate-200">All Assigned Projects ({projects.length})</h2>
                {projects.length === 0 ? (
                  <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 p-10 text-center">
            ...
            {tab === 'notifications' && (
              <div className="space-y-2 max-w-2xl">
                <h2 className="text-lg font-bold text-slate-700 dark:text-slate-200 mb-4">Notifications</h2>
                  ...
                  notifications.map(n => (
                    <div
                      ...
                      className={`p-4 rounded-xl border cursor-pointer transition-colors
                        ${n.read ? 'bg-white dark:bg-slate-800 border-gray-100' : 'bg-teal-50 border-teal-200'}`}
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="text-right flex-shrink-0">
                          ...
                          {!n.read && <span className="inline-block w-2 h-2 bg-teal-500 rounded-full mt-1" />}
                        </div>
                      </div>
                    </div>
                  ))
      ...
      {selectedProject && (
        <ProjectDetailModal
          ...
          theme={theme}
          commentLabel="Add Guidance / Feedback"
...
// ── Project Card ──────────────────────────────────────────────────────────────
const ProjectCard = ({ project, theme, onReview }) => {
    ...
    <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 shadow-sm hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h3 className="font-bold text-slate-800 dark:text-slate-100 truncate">{project.title}</h3>
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            {project.innovator_name} · {project.category}
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={onReview}
            className={`px-3 py-1.5 ${theme.btn} text-white rounded-lg text-xs font-semibold`}
          >
            Review Project
          </button>
        </div>
      ...
      <div className="mt-4">
        <div className="flex items-center justify-between mb-1.5">
          <span className="text-xs text-gray-500 dark:text-slate-400">Milestone Progress</span>
