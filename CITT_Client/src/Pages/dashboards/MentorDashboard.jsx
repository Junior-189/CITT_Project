...
const ReviewMilestoneModal = ({ project, onClose, getAuthenticatedAxios }) => {
  ...
  const handleAction = async (stageNum, action) => {
    ...
    try {
      const api = getAuthenticatedAxios();
      await api.put(`/api/projects/${project.id}/milestones/${stageNum}/${action}`, { notes: notes || 'Approved' });
      ...
      <div className="bg-white rounded-2xl w-full max-w-3xl my-6">
        ...
        <div className="px-6 py-3 border-b">
          ...
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div className="bg-purple-500 h-2 rounded-full" style={{ width: `${Math.round(completedCount/9*100)}%` }} />
          </div>
        </div>
        {msg && (
...
const MentorDashboard = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      try {
        const api = getAuthenticatedAxios();
        const res = await api.get('/api/projects/mentor/projects');
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="max-w-5xl mx-auto">
        <h1 className="text-2xl font-bold text-slate-800 mb-6">Mentor Dashboard</h1>

        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
          {[
            { label: 'Assigned Projects', count: projects.length, color: 'border-purple-500' },
        ...
        <h2 className="text-lg font-semibold text-slate-700 mb-4">My Assigned Projects</h2>
