...
import {
  FolderKanban, BookOpen, MessageSquare, BarChart3, Briefcase,
...
// ── Milestone View Modal (read-only for DEBM) ─────────────────────────────────
...
const DEBMWorkspace = () => {
  ...
  const [bizForm, setBizForm] = useState({ project_id: '', business_name: '', business_plan: '', market_status: '', investment_amount: '', notes: '' });
  ...
  const fetchAll = async () => {
    setLoading(true);
    try {
      const [dash, proj, comp, train, biz] = await Promise.all([
        api().get('/api/departments/DEBM/dashboard'),
        api().get('/api/departments/DEBM/projects?limit=200'),
        api().get('/api/departments/DEBM/complaints'),
        api().get('/api/departments/DEBM/trainings'),
        api().get('/api/departments/DEBM/business-records'),
      ]);
      setDashData(dash.data);
      setProjects(proj.data.projects || []);
      setComplaints(comp.data.complaints || []);
      setTrainings(train.data.trainings || []);
      setBusinessRecords(biz.data.records || []);
  ...
  const handleComplaintUpdate = async (id, updates) => {
    try {
      await api().put(`/api/departments/DEBM/complaints/${id}`, updates);
      flash('success', 'Complaint updated');
      const r = await api().get('/api/departments/DEBM/complaints');
  ...
  const submitComplaint = async () => {
    ...
    try {
      await api().post('/api/departments/DEBM/complaints', complaintForm);
  ...
  const submitTraining = async () => {
    ...
    try {
      await api().post('/api/departments/DEBM/trainings', trainingForm);
      ...
      const r = await api().get('/api/departments/DEBM/trainings');
      setTrainings(r.data.trainings || []);
    } catch { flash('error', 'Failed to create training'); }
  };

  const submitBizRecord = async () => {
    if (!bizForm.project_id) { flash('error', 'Please select a project'); return; }
    try {
      await api().post('/api/departments/DEBM/business-records', { ...bizForm, investment_amount: bizForm.investment_amount || null });
      flash('success', 'Business record created');
      setShowBizForm(false);
      setBizForm({ project_id: '', business_name: '', business_plan: '', market_status: '', investment_amount: '', notes: '' });
      const r = await api().get('/api/departments/DEBM/business-records');
      setBusinessRecords(r.data.records || []);
    } catch { flash('error', 'Failed to create record'); }
  };
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {[
                  ...
                  'Develop business plans for innovations',
                  'Advertising and marketing of innovative products',
                  'Management and financial advisory',
                  'Liaise with worldwide experts in innovation and technology transfer',
                  'Promote Intellectual Property Rights (IPR) awareness',
                  'Maintain database of Entrepreneur and Business activities',
                  'Design mechanisms to convert research outputs to commercial companies',
                  'Handle student Entrepreneurship and Business Management matters',
                  'Attract investment in innovation and commercialization',
                  'Handle student and staff complaints related to DEBM',
                ].map((fn, i) => (
              ...
              <div className="bg-white rounded-2xl border border-blue-200 p-6 mb-5">
                ...
                <div className="space-y-3">
                  ...
                  <textarea rows={4} value={complaintForm.description} onChange={e => setComplaintForm(f => ({ ...f, description: e.target.value }))}
                    className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-400 outline-none resize-none" placeholder="Describe the issue… *" />
                </div>
                <div className="flex gap-2 mt-4">
